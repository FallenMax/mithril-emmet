...
function expand(abbr, { tab = '  ', indentLevel = 0 } = {}) {
  ...
  return indent(output, { skipFirstLine: true, indentLevel })
  ...
  function indent(output, { skipFirstLine = false, indentLevel = 1 } = {}) {
    const arr = Array.isArray(output) ? output : output.split('\n')
    return arr.map((s, i) =>
